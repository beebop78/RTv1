...
/*   Updated: 2015/01/22 19:10:14 by rcargou          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int				newcamera(t_scene *scene, char **str)
{
	...
	scene->camera.cameraray.raypos =
		rcm_vecnormalize(scene->camera.cameraray.raypos);
	scene->camera.cameradist = rcm_vecnorme(scene->camera.cameraray.raypos);
	scene->camera.cameraray.raydir =
		rcm_vecnormalize(rcm_vecneg(scene->camera.cameraray.raypos));
	scene->camera.cameraright = rcm_crossproduct(scene->camera.cameraray.raydir,
		scene->camera.cameraup);
	scene->camera.cameraup = rcm_crossproduct(scene->camera.cameraright,
		scene->camera.cameraray.raydir);
	scene->camera.viewplaneupleft =
		rcm_vecsum(scene->camera.cameraray.raypos,
		(scene->camera.cameraray.raydir));
